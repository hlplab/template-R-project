...
d.test %>%
  ...
  mutate(Response.ProportionVoiceless = ifelse(Response == "voiceless", 1, 0)) %>%
  ggplot(aes(
    x = Item.VOT, 
...
```{r, message=FALSE}
d.test %>%
  group_by(ParticipantID, Item.VOT, Categorization.Intercept, Categorization.Slope) %>%
  summarise(Response.ProportionVoiceless = mean(ifelse(Response == "voiceless", 1, 0))) %>%
  ggplot(
    aes(
      x = Categorization.Intercept, 
      y = Categorization.Slope, 
      color = Response.ProportionVoiceless)) +
  geom_density2d(color = "black") +
  geom_text(aes(label = ParticipantID), alpha = .3) +
