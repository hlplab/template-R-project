...
To get started on using this manuscript, have a look at any of the files with "START-HERE" in the filename. It's the parent Rmd file that loads all the other Rmd files, one for each section of the paper.\todoTFJ{You can also leave comments for yourself or collaborators with the \\todo tag. This particular example uses an author-specific comment, as signaled through the color of the comment box.} 
